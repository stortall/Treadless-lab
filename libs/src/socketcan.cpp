...
namespace scpp {
SocketCan::SocketCan() {}
SocketCanStatus SocketCan::open(const std::string &can_interface,
                                int32_t read_timeout_ms, SocketMode mode) {
  ...
  if ((m_socket = socket(PF_CAN, SOCK_RAW, CAN_RAW)) < 0) {
    perror("socket");
    std::cout << "logger here3 " << std::endl;
  ...
  if (mode == MODE_CANFD_MTU) {
    ...
    if (ioctl(m_socket, SIOCGIFMTU, &ifr) < 0) {
      ...
      std::cout << "logger here13 " << std::endl;
    }
    mtu = ifr.ifr_mtu;

    if (mtu != CANFD_MTU) {
      return STATUS_CANFD_NOT_SUPPORTED;
      std::cout << "logger here23 " << std::endl;
    }

    /* interface is ok - try to switch the socket into CAN FD mode */
    if (setsockopt(m_socket, SOL_CAN_RAW, CAN_RAW_FD_FRAMES, &enable_canfd,
                   sizeof(enable_canfd))) {
      std::cout << "logger here33 " << std::endl;
  ...
  //    perror("setsockopt SO_TIMESTAMPING is not supported by your Linux
  //    kernel");
  //}
  ...
  if (bind(m_socket, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    ...
    std::cout << "logger here43 " << std::endl;
  }

  return STATUS_OK;
  std::cout << "logger here35 " << std::endl;
}
...
SocketCanStatus SocketCan::read(CanFrame &msg) {
  ...
  if (num_bytes != CAN_MTU && num_bytes != CANFD_MTU) {
    ...
    std::cout << "logger here84 " << std::endl;
  }
